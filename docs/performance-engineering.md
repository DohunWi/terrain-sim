...
4. **`stepRigidBody`의 스레드 확장 한계(~3.5배, 물리 코어 8개 기준)는 소프트웨어로 고칠 수 있는 병목이라는 증거를 찾지 못했다** — 이 프로젝트 개발 머신(Apple Silicon, 물리/논리 코어 8개, P/E 코어 비대칭)의 현실적인 병렬성 천장으로 받아들인다.
